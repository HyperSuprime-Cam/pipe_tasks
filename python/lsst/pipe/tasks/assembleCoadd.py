...
\ref SafeClipAssembleCoaddTask_ "SafeClipAssembleCoaddTask" rather than AssembleCoaddTask to make the coadd.
    ...
    def run(self, dataRef, selectDataList=[]):
        ...
        returnStruct = pipeBase.Struct(coaddExposure=None,
                                       coaddExposurePsfMatched=None)
        for warpType in self.getWarpTypeList():
            # self.warpType used by all instance methods to determine which warp type to process
    ...
    def runWarpType(self, dataRef, calExpRefList, skyInfo):
        ...
        if self.config.doWrite:
            self.writeCoaddOutput(dataRef, coaddExp, warpType=self.warpType)
